...
export default function Header() {
  ...
  /* measure drawer height dynamically */
  useEffect(() => {
    if (!drawerRef.current) return;
    if (open) {
      setDrawerH(drawerRef.current.scrollHeight);
    } else {
      setDrawerH(0);
    }
  }, [open, user]);
  ...
  useEffect(() => {
    const fn = () => {
      ...
      if (cur < 8) {
        ...
        setScrolled(true);
        if (cur > lastScrollY.current + 4) { setHidden(true); setOpen(false); }
        else if (cur < lastScrollY.current - 4) { setHidden(false); }
      }
    ...
    <div className={isHome ? "hero-full-height flex flex-col" : "flex flex-col"}>
      ...
      <header
        ...
        style={{
          ...
          background: "#fff",
          boxShadow: scrolled ? "0 4px 24px rgba(0,0,0,0.09)" : "0 1px 0 #e5e7eb",
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 0 }}>
              <span style={{
                ...
                paddingRight: 14, borderRight: "1px solid rgba(255,255,255,0.2)",
              }}>
                <LuMapPin size={12} style={{ flexShrink: 0 }} />
                Toshkent, O'zbekiston
              </span>
              <span className="hidden-sm" style={{
                display: "flex", alignItems: "center", gap: 5,
                fontSize: 12, color: "rgba(255,255,255,0.88)", fontWeight: 500,
                padding: "0 14px", borderRight: "1px solid rgba(255,255,255,0.2)",
              }}>
                <span style={{ background: "#fff", color: "#dc2626", fontSize: 9, fontWeight: 800, letterSpacing: "0.8px", padding: "1px 5px", borderRadius: 4, lineHeight: "14px" }}>LIVE</span>
                <LuFlame size={12} />
                500+ tasdiqlangan bloger
              </span>
              <span className="hidden-md" style={{
                display: "flex", alignItems: "center", gap: 5,
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <>
                  ...
                  >
                    ...
                    {unreadCount > 0 && (
                      <span style={{
                        position: "absolute",
                        top: unreadCount > 9 ? 2 : 3, right: unreadCount > 9 ? 2 : 3,
                        minWidth: unreadCount > 9 ? 16 : 13, height: unreadCount > 9 ? 16 : 13,
              ...
              {user ? (
                  ...
                  {userMenuOpen && (
                    <div style={{
                      ...
                      <div style={{ padding: "12px 14px", borderBottom: "1px solid #f1f5f9" }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: "#111" }}>{user.firstName} {user.lastName}</div>
                      ...
                      <button onClick={handleLogout}
                        style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", border: "none", background: "none", cursor: "pointer", color: "#dc2626", fontSize: 13, fontWeight: 600, borderTop: "1px solid #fef2f2" }}
                ...
                <>
                  <span className="hidden-sm" style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: "rgba(255,255,255,0.7)" }}>
        ...
        <nav style={{
          ...
          <ul className="desktop-nav" style={{ display: "flex", alignItems: "center", gap: 2, listStyle: "none", margin: 0, padding: 0, flexShrink: 0 }}>
            {NAV.map(({ to, label }) => (
              <li key={to}>
                <NavLink to={to}
                  style={({ isActive }) => ({ textDecoration: "none", fontSize: 13.5, fontWeight: 600, padding: "7px 11px", borderRadius: 9, color: isActive ? "#dc2626" : "#374151", background: isActive ? "#fef2f2" : "transparent", display: "block", whiteSpace: "nowrap", transition: "color 0.15s, background 0.15s" })}
                  onMouseEnter={e => { if (!e.currentTarget.style.background.includes("fef2f2")) { e.currentTarget.style.color = "#dc2626"; e.currentTarget.style.background = "#fff5f5"; } }}
                  onMouseLeave={e => { if (!e.currentTarget.style.background.includes("fef2f2")) { e.currentTarget.style.color = "#374151"; e.currentTarget.style.background = "transparent"; } }}
                >
                  {label}
                </NavLink>
              </li>
            ...
            <li style={{ marginLeft: 6 }}>
              ...
              >
                <LuPlus size={14} strokeWidth={2.5} /> E'lon berish
              </NavLink>
            </li>
          </ul>
          ...
          <button
            ...
            style={{
              display: "none",
              background: open ? "#fef2f2" : "none",
              border: "1.5px solid",
              borderColor: open ? "#fecaca" : "#e5e7eb",
              cursor: "pointer",
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {NAV.map(({ to, label, Icon }) => (
                ...
                >
                  {({ isActive }) => (
                    <>
                      <span style={{ width: 32, height: 32, background: isActive ? "#fecaca" : "#f3f4f6", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "background 0.15s" }}>
              ...
              {user ? (
                <>
                  ...
                  <button onClick={() => { handleLogout(); setOpen(false); }}
                    style={{ flex: 1, border: "1.5px solid #fee2e2", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "12px", borderRadius: 11, fontSize: 14, fontWeight: 600, color: "#dc2626", background: "#fef2f2", cursor: "pointer" }}
                  >
